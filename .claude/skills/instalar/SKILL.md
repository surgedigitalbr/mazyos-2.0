...
## Fase 5.5 — Desacoplar do molde (git limpo)

O projeto foi clonado do MazyOS, então carrega o histórico e o remote do molde (`mazzeoia/MazyOS`). Cada cliente tem que ser um repo limpo e independente. Conferir com `git remote get-url origin`.

Se o origin ainda for o molde (ou houver histórico do molde), oferecer:

> "Esse projeto ainda tá ligado ao repositório-molde do MazyOS. Quer que eu desacople agora pra ele virar um repo limpo só desse cliente? (apaga só o histórico do molde, mantém todos os arquivos). Você ainda pode salvar no seu GitHub depois com `/salvar`."

Se sim, com confirmação (é destrutivo no histórico, não nos arquivos):
```bash
rm -rf .git
git init
git add .
git commit -m "Setup inicial — <nome do negócio>"
git branch -M main
```

Não criar remote aqui — isso fica pro `/salvar`, quando o usuário for publicar no repo dele. Se preferir deixar pra depois, tudo bem: o próprio `/salvar` detecta o molde e desacopla na primeira vez (Caso B).
